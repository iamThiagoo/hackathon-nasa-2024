...
export class NEObject {
  ...
  constructor(
    ...
    this._sceneObject = new THREE.Mesh(
      this._sceneObjectGeometry,
      this._sceneObjectMaterial
    );
  ...
  get _sceneObjectMaterial() {
    ...
    const texture = textureLoader.load("/img/" + this.textureSource);
    return new THREE.MeshBasicMaterial({ map: texture });
  }

  get scaledRadius() {
    return this.diameter / 2 / planetScale;
  }
  ...
  setOrbit() {
    this.orbitCurve = new THREE.EllipseCurve(
      0,
      0, // Ponto central (x, y)
      this.earthDistance + 5,
      this.earthDistance + 5, // Raio (xRadius, yRadius)
      0,
  ...
  animate() {
    ...
    const inclinedY =
      point.y * Math.cos(THREE.MathUtils.degToRad(this.inclination));
    const inclinedZ =
      point.y * Math.sin(THREE.MathUtils.degToRad(this.inclination));
